...
public class ELServlet extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       *    서버에서 session이 만들어지면 요청한 브라우저의 쿠키에 ID를 보내서 브라우저가 서버에 요청시 요청데이터와 쿠키에 저장된 ID를 같이 보내 서버가 브라우저를 구별함
       *    
       * 3. HttpServletRequest
       *    요청때마다 매번 생성되는 객체
       *    이 영역에 데이터를 담으면 해당 request객체를 포워딩받는 응답 jsp에서만 가능
       ...
       * 	데이터를 꺼낼 때 -> .getAttribute('키',값)
       * 	데이터를 삭제할 때 -> .removeAttribute('키',값)
       * 
       */
      ...
      application.setAttribute("scope", "session");
      
      // request scope
      request.setAttribute("classRoom", "I강의장");
      request.setAttribute("student", new Person("최지원", 20, "남자"));
      application.setAttribute("scope", "request");
      
      // 응답페이지를 지정해서 해당 페이지가 포워딩되도록 설정

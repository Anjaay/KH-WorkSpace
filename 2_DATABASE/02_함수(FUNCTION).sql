...
/*
    <ODER BY절>
    SELECT로 가장 마지막 줄에 작성, 실행순서 또한 가장 마지막에 실행
    
    [표현법]
    SELCET 조회할 컬럼...
    FROM 조회할 테이블
    WHERE 조건식
    ORDER BY 정렬기준될 컬럼 | 병칭 | 컬럼순번 [ASC | DESC] [NULL FIRST | NULL LAST]
    
    - ASC : 오름차순(작은 값으로 시작해서 값이 점점 커지는 것) -> 기본값
    - DESC : 내림차순(큰값으로 시작해서 값이 점점 줄어드는 것)
    
    -- NULL은 기본적으로 가장 큰값으로 분류해서 정렬한다.
    = NULLS FIRST : 정렬하고자 하는 컴럼값에 NULL이 있을 경우 해당데이터 맨 앞에 배치(DESC일때 기본값)
...
SELECT INSTR('AABAACAABBAA', 'B', -1) FROM DUAL; -- 뒤에서 부터 찾고 위치를 읽을때는 앞에서부터 읽어준다.
...
/*
    *LTRIM/RTIRM
    ...
    LTRIM/RTIRM(STRING, [제거하고자하는 문자들])
    
    문자열의 왼쪽 혹은 오른쪽에서 제거하고자하는 '문자들'을 찾아서 제거한 나머지 문자열 반환
*/
...
SELECT TRIM('Z' FROM 'ZZZZKHZZZZZZZ') FROM DUAL;

SELECT TRIM(LEADING 'Z' FROM 'ZZZZKHZZZZZZZ') FROM DUAL;
SELECT TRIM(TRAILING 'Z' FROM 'ZZZZKHZZZZZZZ') FROM DUAL;
...
SELECT ROUND(123.456) FROM DUAL; -- 기본자릿수는 소수점 첫번째 자리에서 반올림
...
-- BONUS가 있고 여자인 사원의
-- 이름, 주민등록 번호, 직급코드, 부서코드, 급여, 보너스를 조회하고싶다.

SELECT EMP_NAME, EMP_NO, JOB_CODE, DEPT_CODE, SALARY, BONUS
FROM EMPLOYEE
WHERE (JOB_CODE = 'J7' OR JOB_CODE = 'J6') AND SALARY >= 2000000 AND BONUS IS NOT NULL AND
...
/*
    *EXTRACT :  특정 날짜로 부터 년|월|일 추출해서 반환하는 함수
...
SELECT TO_CHAR(2000000, 'L9999999') FROM DUAL; -- 현재 설정된 나라의 로컬 화폐 단위 나타남
...
SELECT TO_DATE(240807) FROM DUAL; -- 50년 미만은 자동으로 20XX으로 설정 50년 이상은 19XX으러 설정된다.
...
-- 2. AVG(NUMBER) : 해당 컬럼값들의 평균울 구해서 반환
...
-- 4. MAX(모든타입가능) : 해당 컬럼값등 중 가장 큰 값을 구해서 반환

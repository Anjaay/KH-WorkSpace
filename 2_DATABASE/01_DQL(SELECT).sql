...
-- 코드 실행시 현재시간을 표시하는 상수 : SYSDATE 아
...
/*
    <컬럼명의 별칭을 지정하기>
    산술연산을 하게되면 컬럼명이 지저분해진다. 이떄 컬러명에 별칭을 부여해서 깔끔하게 가져올 수 있다.
    (표현법)
    컬러명 별칭 / 컬럼명 AS 별칭 / 컬러명 "별칭" / 컬럼명 AS "별칭"
*/
...
-- 위처럼 작성시 에러가 발생한다. DISTINCT는 한번만 사용이 가능하다.
...
-- EMPLOYEE 에서 부서코드가 'D5'인 사람들만 조회 (모든컬럼)
SELECT * 
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
...
SELECT EMP_NAME, EMP_ID, SALARY
FROM EMPLOYEE
-- WHERE SALARY >= 3500000 AND SALARY <= 600000;
WHERE SALARY BETWEEN 3500000 AND 600000;

-- 350만원 미만, 600만원 초과 모든 사원의 사원명, 사번 , 급여조회
SELECT EMP_NAME, EMP_ID, SALARY
FROM EMPLOYEE
-- WHERE SALARY < 3500000 OR SALARY > 600000;
...
/*
    ...
    특덩패턴을 제시할 때 와일드카드라는 특정패턴이 정의되어야한다.
    1. '%' : 포함문자 검색(0글자 이상 전부 조회)
     EX) 비교할 대상 컬럼 LIKE '문자%' : 비교할대상컬럼값 중에서 해당 문자로 시작하는 것들만 가져온다.
         비교할 대상 컬럼 LIKE '%문자' : 비교할대상컬럼값 중에서 해당 문자로 끝나는 것들만 가져온다.
         비교할 대상 컬럼 LIKE '%문자%' : 비교할대상컬럼값 중에서 해당 문자로 포함된 값 조회.
         
    2. '_' : 1글자를 대체하는 검색
       EX) 비교할 대상 컬럼 LIKE '_문자' : 비교대상 컬럼값 문자앞에 아무글자나 한글자가 있는 값을 조회
           비교할 대상 컬럼 LIKE '문자_' : 비교대상 컬럼값 문자뒤에 아무글자나 한글자가 있는 값을 조회
           비교할 대상 컬럼 LIKE '_문자_' : 비교대상 컬럼값 문자앞뒤에 아무글자나 한글자가 있는 값을 조회
...
FROM DEPARTMENT
WHERE DEPT_TITLE LIKE '해외영업_%부';

--==============================================================================
/*
    <IN>
    WHERE절에서 비교대상 컬럼값이 내가 제시한 목록중에 일치하는 값ㅇ 있는지 검사
...
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
-- WHERE DEPY_CODE = 'D6' OR DEPT_CODE = 'D8' OR DEPT_CODE = 'D5';

/*
    ...
    임시테이블 같은 존재(실제 데이터가 담뎌있는건 아니다 -> 논리테이블)
*/
...
/*
    *DML 명령어로 조작이 불가능한 경우가 많다
    ...
    2) 뷰에 정의되어있지 않은 컬름 중에 베이스테이블 상에 NOT NULL제약조건이 지정되어있는 경우
...
/*
    ...
    1) OP REPLACE : 기존에 동일한 뷰가 있을 경우 갱신하고, 없을 경우 새로 생성해라
    2) FORCE | NOFORCE
        > FORCE : 서브쿼리에 기술된 테이블이 존재하지 않아도 뷰가 생성되도록해라
        > NOFORCE : 서브쿼리에 기술된 테이블이 존재하는 테이블이여야만 한다(기본값)
    3) WITH CHECK OPTION : DML시 서브쿼리에 기술된 조건에 부합한 DML이 가능하도록
    4) WIRH READ ONLY : 뷰에 대해서 조회만 가능하도록
*/

...
-- 2) 대리직급임에도 과장 직급 급여들 중 최소 급여보다 많이 받는 사원들의 사번, 이름, 직급, 급여 조회
-- 2_1) 과장 직급 급여들
SELECT SALARY
FROM EMPLOYEE
...
/*
    3. 다중열 서브쿼리
    결과값으 한 행이지만 나열된 컬럼수가 여러개일 경우
*/

-- 1) 하이유 사원과 같은 부서코드, 같은 직급코드에 해당하는
-- (사원명, 부서코드, 직급코드, 입사일)
--> 단일행 서브쿼리
...
-- 다중열 서브쿼리
SELECR EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE
WHERE (DAPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
                                FROM EMPLOYEE
                                WHERE EMP_NAME = '하이유');
        
-- 박나리 사원과 같은 직급켜드, 같은 사수를 가지고 있는 사원들의 사번, 사원명, 직급코드, 사수번호
SELECT EMP_NAME, JOB_CODE, MANAGER_ID
FROM EMPLOYEE
WHERE(JOB_CODE MANAGER_ID) = (SELECT JOB_CODEM, MANAGER_ID
...
HAVING SUM(SALARY) > (SELECT SUM(SALARY) * 0.2
                            FROM EMPLOYEE);
-- 2. 나이 상 가장 막내의 사원코드, 사원 명, 나이, 부사명, 직급 명 조회
...
/*
    *순위를 매기는 함수(WINDOW FUNCTION)
    ...
    DANSE_RANK OVER(정렬기준) : 동일한 순위가 있다고 해도 그다음 등수를 무조건 1씩 증가
